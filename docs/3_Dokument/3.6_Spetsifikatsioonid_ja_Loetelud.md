...
* **3.6.3.1. Tüüpilised Loetelud Elektriprojektis:**
    ...
    * **Magistraalkaablite loetelu:** Kaabli tähis, tüüp/mark, soonte arv ja ristlõige, hinnanguline pikkus, alguspunkt (kilp/klemm), lõpp-punkt (seade/karp/klemm). (Koostatakse tavaliselt TP staadiumis). Magistraalkaablite kogused esitatakse projektis.
    * **Põhiseadmete loetelu:** Suuremate seadmete (trafod, generaatorid, UPSid jne) loetelu koos põhiparameetrite ja kogustega.

    !!! info "Paigalduskaablite kogused"
        Paigalduskaablite (grupijuhtmete) koguseid põhi- ega tööprojektis ei esitata — need arvestab ehitustöö töövõtja projektdokumentatsiooni alusel. Projektis esitatakse magistraalkaablite kogused.
    * **Installatsioonimaterjalide loetelu:** Muude materjalide (nt pistikupesad) koondkogused.
...
* **BIM Mudel:** BIM-põhises projekteerimises tuleks spetsifikatsioonid (parameetritena) ja loetelud/mahud **genereerida otse mudelist** (vt ptk 6.6). See tagab andmete järjepidevuse ja vähendab käsitsi tehtavaid vigu. Mudeli kvaliteet (LOD, parameetrite korrektsus) on siin määrava tähtsusega.
