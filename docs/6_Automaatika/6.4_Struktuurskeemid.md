...
<figure markdown="span">
  ...
  <figcaption>Joonis 1. Hooneautomaatika struktuurskeem — põhiprojekti staadium</figcaption>
</figure>
...
<figure markdown="span">
  ...
  <figcaption>Joonis 2. Hooneautomaatika struktuurskeem — tööprojekti staadium</figcaption>
</figure>

#### 6.4.2. Arvestite struktuurskeem
...
Arvestite süsteemi kirjeldus koosneb struktuurskeemist, arvestite tabelist ning vajadusel täiendavatest joonistest ja tabelitest (nt kaablitabelid ja automaatika plaanid).

Struktuurskeem esitab arvestite andmeedastuse ülesehituse põhimõttelisel kujul. Detailsem info arvestite kohta (nt unikaalne tähis, asukoht, teeninduspiirkond, liini kuuluvus, sideprotokoll jne) esitatakse vastavalt projekteerimisstaadiumile arvestite tabelis, millele tuleb skeemil viidata.

Kaabelduse ja ühenduste täiendav info esitatakse vastavalt projekteerimisstaadiumile kaablitabelites ja automaatika plaanidel.

Struktuurskeemil nõutud info võib olla osaliselt esitatud viidetena tabelitele ja joonistele — kogu detailne info ei pea olema skeemil, kui lahendus on tervikuna üheselt arusaadav.

Oluline on, et kogu süsteemi kirjeldus oleks tervikuna üheselt mõistetav, sõltumata info jaotusest skeemi, tabelite ja jooniste vahel.
...
* *Märkus: Skeemil tuleks viidata arvestite tabelile.

<figure markdown="span">
  ![Arvestite struktuurskeem PP](../_assets/media/6-4_Arvestid_PP.jpg)
  <figcaption>Joonis 1. Arvestite struktuurskeem — põhiprojekti staadium</figcaption>
</figure>
...
* *Märkus: Skeemil tuleb viidata arvestite tabelile, mis sisaldab süsteemi liidetud arvestite detailset infot. Kaabelduse infot täiendavad kaablitabel ja automaatika plaanid.*

<figure markdown="span">
  ![Arvestite struktuurskeem TP](../_assets/media/6-4_Arvestid_TP.jpg)
  <figcaption>Joonis 2. Arvestite struktuurskeem — tööprojekti staadium</figcaption>
</figure>

#### 6.4.3 Tehnosüsteemide (KVJ, VK jne) funktsionaalskeemid

Funktsionaalskeemid (EVS kohaselt tehnosüsteemide toimimisskeemid) kirjeldavad detailselt, kuidas konkreetne tehnosüsteem (küte, ventilatsioon, jahutus — KVJ; veevarustus ja kanalisatsioon — VK jne) automaatikasüsteemi poolt juhitakse ja reguleeritakse.
...
* Näidatakse iga mõõte- ja juhtimisseadme I/O punktid (füüsilised ja virtuaalsed) .
...
* Esitatakse mõõte- ja reguleerimispunktide põhiparameetrid (vt ptk 6.6 tabelis 1 minimaalselt I/O punktide osa, seadesuuruste osa soovituslik) .

<figure markdown="span">
  ![KVJ funktsionaalskeem PP](../_assets/media/6-4_Vent_skeem_PP.jpg)
  <figcaption>Joonis 1. Ventilatsiooniseadme funktsionaalskeem — põhiprojekti staadium (ilma tehaseautomaatikata)</figcaption>
</figure>
...
* Näidata seadme andmeside liidese kaudu sidumise põhimõte ja minimaalne/eeldatav I/O maht (punktide arv) ning põhiline vahetatav info .
...
* I/O-punktid ja nende liigid vastavalt kasutatavale kontrollerile (füüsilised/virtuaalsed, sisendid/väljundid/reguleerimissignaalid) .
...
* **Alakeskuse kaudu juhitavate süsteemide puhul:**
    * Näidata iga mõõte- ja juhtimisseadme I/O punktid (füüsilised ja virtuaalsed).
    * Näidata ühendused ruumiseadmete, alakeskuste, seadmete ja elektrikilpide vahel ning seosed elektrikilpide ja alakeskuse vahel.
* **Ruumikontrolleritega ruumide korral:**
    * Esitada erinevad kontrolleritüübid, eristatud tähistega (nt TC20, TC21, TC25).
    * Näidata kontrollerite konfiguratsioonid:
        * integreeritud andurid (ruumitemperatuur, CO₂, niiskus)
        * juhitavad seadmed (fancoil, jahutuspalk, kütteradiaatorid, VAV jne)
        * juhtimisviisid (on/off, PWM, 0–10 V)
        * kasutatav andmesideprotokoll (BACnet, Modbus jt)
    * Kontrollerite I/O maht peab olema tuletatav tüüpsest funktsionaalskeemist.
    * Sama tähisega kontrollerid (nt TC20) peavad olema funktsionaalselt identsed.
* Esitada skeemi tööpõhimõtte kirjeldus.
* Määrata mõõte- ja reguleerimispunktide parameetrid (tööpõhimõtte kirjelduses või eraldi tabelina).
* *Märkus: Skeemil tuleb viidata ruumikliima seadmetabelile (vt ptk 6.6), kui tabel on koostatud.*

**Tüüpskeemide näited:**

<figure markdown="span">
  ![Kliima PP fancoil](../_assets/media/6-4_Kliima_PP_fancoil.jpg)
  <figcaption>Joonis 1. Jahutuse fancoil + kütteradiaator</figcaption>
</figure>

<figure markdown="span">
  ![Kliima PP paneelid](../_assets/media/6-4_Kliima_PP_paneelid.jpg)
  <figcaption>Joonis 2. Lae kütte-/jahutuspaneelid + VAV</figcaption>
</figure>

<figure markdown="span">
  ![Kliima PP split](../_assets/media/6-4_Kliima_PP_split.jpg)
  <figcaption>Joonis 3. Split-seadmed</figcaption>
</figure>

**TP (Tööprojekti staadium):**

* Täpsustatakse PP skeemid vastavalt valitud seadmetele, ühendustele ja parameetritele.
* Täpsustada I/O punktid vastavalt kasutatavale kontrollerile (füüsilised/virtuaalsed, signaalitüübid - 0–10 V, digitaalne jne).
...
* *Märkus: Skeemil tuleb viidata ruumikliima seadmetabelile (vt ptk 6.6), mis sisaldab iga ruumi kohta kasutatavate seadmete ja I/O punktide detailset infot.*

#### 6.4.5. Valgustuse juhtimise funktsionaalskeemid

Kirjeldavad valgustussüsteemide automaatse juhtimise lahendusi (nt DALI, KNX, füüsilised I/O-d). Valgustuse juhtimise lahendused ei pruugi olla esitatud eraldiseisva funktsionaalskeemina. Sõltuvalt projekti ulatusest ja valitud tehnilisest lahendusest võib valgustuse juhtimine olla kajastatud kas eraldi skeemina, hooneautomaatika andmeside struktuurskeemi osana või üldise I/O punktide skeemi osana (vt ptk 6.4.6).

DALI, DMX ja muude sarnaste süsteemide puhul käsitletakse valgustuse juhtimist tavapäraselt vastava eriala projektis (nt tugevvool või eraldi valgustuse projekt) ning hooneautomaatikas käsitletakse neid süsteeme vajadusel andmeside liidese kaudu integreeritavate lahendustena.

Sõltumata skeemi esitamise viisist peab valgustuse juhtimise loogika ja tööpõhimõte olema üheselt arusaadavalt esitatud.
...
* Iga mõõte- ja juhtimisseadme I/O punktid (füüsilised ja virtuaalsed) .
* Ühendused ruumiseadmete, alakeskuste, seadmete ja elektrikilpide vahel .
* Skeemi tööpõhimõtte kirjeldus .
* Mõõte- ja reguleerimispunktide põhiparameetrid (nt tabelis 1, vt ptk 6.6 , I/O punktide osa, seadesuuruste osa soovituslik) .
* Andmeside võrgupõhiste valgustuse juhtimise lahenduste (KNX, DALI vms) sidumise korral esitada lisaks: andmeside liidese kaudu sidumise põhimõte ja sidumise eeldatav I/O maht (visualiseeritavate punktide juhtimise ja jälgimise maht) .

<figure markdown="span">
  ![Valgustuse juhtimine füüsiliste I/O-dega](../_assets/media/6-4_Valgustus_IO.jpg)
  <figcaption>Joonis 1. Füüsiliste I/O-de kaudu valgustuse juhtimine</figcaption>
</figure>

<figure markdown="span">
  ![DALI valgustusjuhtimissüsteemi integreerimine](../_assets/media/6-4_Valg_DALI.jpg)
  <figcaption>Joonis 2. DALI valgustusjuhtimissüsteemi integreerimine</figcaption>
</figure>
...
* I/O-punktide täpsustus vastavalt kasutatavale kontrollerile (füüsilised/virtuaalsed, signaalitüüp) .
...
#### 6.4.6 Eripunktide skeem (üldised I/O punktid)
...
* Skeemi tööpõhimõtte kirjeldus .
* Mõõte- ja reguleerimispunktide põhiparameetrid (nt tabelis 1, vt ptk 6.6 , I/O punktide osa, seadesuuruste osa soovituslik) .
* **Tehaseautomaatikaga seadme korral:** Näidata hooneautomaatikaga seotud füüsilised punktid (nt tööluba, olek, üldhäire), andmeside liidese kaudu sidumise põhimõte ja I/O maht .

<figure markdown="span">
  ![Üldised I/O-punktid](../_assets/media/6-4_YldIO.jpg)
  <figcaption>Joonis 1. Üldiste I/O-punktide skeem</figcaption>
</figure>
...
* **Andmeside võrgupõhiste lahenduste korral:** Näidata hooneautomaatikaga seotud füüsilised punktid, andmeside liidese kaudu sidumise põhimõte ja I/O maht .